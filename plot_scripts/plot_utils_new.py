...
def plot_tens(fig, q_tilde, X_0, P_mat, P_mat_per, l_b, n_N, n_G, ghost_nodes,
    ...
    X_sur = np.zeros((3*(n_P - 6*2*(nz+nx+2) - 2*2*(nz+nx))))
    Y_sur = np.zeros((3*(n_P - 6*2*(nz+nx+2) - 2*2*(nz+nx))))
    Z_sur = np.zeros((3*(n_P - 6*2*(nz+nx+2) - 2*2*(nz+nx))))
    ...
    for i in range(n_P - 6*2*(nz+nx+2) - 2*2*(nz+nx)): #n_P - 6*2*(nz+nx+2) - 2*2*(nz+nx)
        ...
        X_sur[3*i:3*i+3] = [X_A, X_B, X_C]
        Y_sur[3*i:3*i+3] = [Y_A, Y_B, Y_C]
        Z_sur[3*i:3*i+3] = [Z_A, Z_B, Z_C]
    ...
    surf = ax.plot_trisurf(Z_sur, X_sur, Y_sur, cmap="viridis", linewidth=0, antialiased=False)

    # n_prova = 100

    # X_sur = np.zeros((3*n_prova))
    # Y_sur = np.zeros((3*n_prova))
    # Z_sur = np.zeros((3*n_prova))

    # for i in range(n_prova): #n_P - 6*2*(nz+nx+2) - 2*2*(nz+nx)
    #     ind_A = P_mat[i, 0]
    #     ind_B = P_mat[i, 1]
    #     ind_C = P_mat[i, 2]
    ...
    # surf = ax.plot_trisurf(Z_sur, X_sur, Y_sur, cmap="viridis", linewidth=0)
    # print(Y_sur)

    # surf = ax.plot_trisurf(Z_sur, X_sur, Y_sur, cmap="viridis", linewidth=0, antialiased=False, 
    #                 vmin=-k_value, vmax=k_value)
    ...
    cbar = fig.colorbar(surf, shrink=0.5, aspect=5, label="$\\Delta z_w$")
